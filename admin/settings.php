                        ...
                        <div class="account-wrapper">
                            <div class="columns">
                                ...
                                if(isset($_POST["update_account"])){
                                    ...
                                    try{
                                        $check_user_password = $connect->prepare("SELECT * FROM admin WHERE username=? AND password=?");
                                        $check_user_password->execute([$user,md5($old_password)]);
                                        $count_cybers = $check_user_password->rowCount();
                                        if($count_cybers == 1){
                                            //Change Password
                                            $update_password_query = $connect->prepare("UPDATE admin SET password=? WHERE username=?");
